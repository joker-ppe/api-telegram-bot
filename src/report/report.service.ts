...
export class ReportService implements OnModuleInit {
  ...
  async handleCronLocal() {
    ...
    if (this.isRunningCron) {
      console.log(
        '[fetchAndStoreBetsLocal] Cron job is running. Skip this time.',
      );
      return;
    }
    ...
    try {
      ...
      if (process.env.INSTANCE_ROLE !== 'cron') {
        ...
        if (currentHour < 18 || (currentHour === 18 && currentMinute <= 30)) {
          console.log(
            `[fetchAndStoreBetsLocal] Fetching 'fetchAndStoreBetsLocal'.  Current time is ${currentHour}:${currentMinute}`,
          );
          // Thực hiện hành động nếu thời gian hiện tại nằm trong khoảng từ 00:00 đến 18:30
          await this.fetchAndStoreBetsLocal();
        } else {
          console.log(
            `[fetchAndStoreBetsLocal] Not in time 'fetchAndStoreBetsLocal'. Current time is ${currentHour}:${currentMinute}`,
          );
          ...
          if (dataDate && dataDate.done === false) {
            try {
              ...
              if (
                ...
                console.log('[fetchAndStoreBetsLocal] Data is synced');
                const todayResult =
                ...
                if (todayResult && todayResult.done) {
                  console.log('[fetchAndStoreBetsLocal] Today result is done');
                  ...
                  console.log(
                    '[fetchAndStoreBetsLocal] Today result is not done',
                  );
                }
              } else {
                console.log('[fetchAndStoreBetsLocal] Data is not synced');
                await this.fetchAndStoreBetsLocal();
              }
            } catch (e) {
              console.error(e);
            ...
            console.log(`[fetchAndStoreBetsLocal] 'fetchAndStoreBets' is done`);
          } else {
            console.log('[fetchAndStoreBetsLocal] Data is not synced');
            await this.fetchAndStoreBetsLocal();
          }
      ...
      this.isRunningCron = false;
      console.log(
        '[fetchAndStoreBetsLocal] ===========> Done cron job\n================================',
      );
    }
  ...
  async fetchAndStoreBetsLocal() {
    try {
      ...
      await this.getWinLoseCronLocal(weekInfo.startDate, currentDateString);

      // await this.getWinLoseCron(lastWeekInfo.startDate, currentDateString);

      // await this.getWinLoseCron('2024-01-01', currentDateString);

      console.log('[fetchAndStoreBetsLocal] ################################');
  ...
  async getWinLoseCronLocal(startDate: string, endDate: string) {
    console.log(`Current date: ${this.getCurrentDateString()}`);
